...
//! clap command tree — generated and custom commands alike — passes through
//! `present` before parsing, so help templates, styles, banners, about text,
...
pub fn present(command: clap::Command) -> clap::Command {
    command.about("Petstore agent CLI (styled by src/presentation.rs)")
}
